...
items <- unfactor(data[7:56]) # please select the columns of your GEB-items in the dataset by changing the expression --- data[...] ---
...
# If this is the case, please uncomment, adapt and run the command row 58 to recode e.g. -99 to NA. 

itemsTest <- as.data.frame(items)
str(itemsTest)
rm(itemsTest)

# items[c(    )] <- as.data.frame(sapply(items[c(     )], recode, " '-99'=NA"))
...
# the item (representing pro-environmental reponses).
# Inverse dichotomous items are recoded into 0 for agreement with item and 1 for disagreement 
# with the item (representing pro-environmental reponses).
############
...
# of your own dataset by changing the numbers which represent the columns of your items in command rows 86 and 89. --- items[c(...)] ---
############
...
RaschModel <- RM(items) # this might take a 20-30 minutes
...
person_estimates <- data[1] #please select the column of the subjects' IDs in the orginial dataset
...
person_estimates['GEB.est'] <- person.parameter(RaschModel)$theta.table$'Person Parameter'
...
write.csv2(person_estimates, "person_estimates_per_person.csv")
write.csv2(person_adds, "person_estimates_summary.csv")
write.csv2(item_estimates, "item_estimates_per_item.csv")
write.csv2(item_adds, "item_estimates_summary.csv")
